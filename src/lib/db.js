...
export async function signUpAdmin({ email, password, nomeUsuario, nomeEmpresa }) {
  ...
  if (!data.session) {
    // E-mail de confirmação é obrigatório nesse projeto. Guardamos o
    // nome da empresa localmente pra caso a pessoa confirme e entre
    // pelo mesmo navegador (completarCadastroEmpresaSeNecessario cuida
    // disso sozinho). Se for outro navegador/aparelho, pedimos de novo.
    try {
      window.localStorage.setItem("j27_pending_signup", JSON.stringify({ nomeUsuario, nomeEmpresa }));
    } catch {}
  ...
  return { pendingConfirmation: false, session: data.session };
}

// Chamada quando a pessoa já está logada (inclusive depois de confirmar
// o e-mail) mas ainda não tem empresa/profile — completa o cadastro.
export async function completarCadastroEmpresa({ nomeEmpresa, nomeUsuario }) {
  const { error } = await supabase.rpc("criar_empresa_e_admin", {
    nome_empresa: nomeEmpresa,
    nome_usuario: nomeUsuario,
  });
  if (error) throw error;
  try { window.localStorage.removeItem("j27_pending_signup"); } catch {}
}

// Tenta completar sozinho usando o que foi salvo no localStorage (mesmo
// navegador). Retorna true se conseguiu, false se precisa perguntar de novo.
export async function tentarCompletarCadastroAutomatico() {
  try {
    const raw = window.localStorage.getItem("j27_pending_signup");
    if (!raw) return false;
    const { nomeUsuario, nomeEmpresa } = JSON.parse(raw);
    if (!nomeEmpresa) return false;
    await completarCadastroEmpresa({ nomeEmpresa, nomeUsuario });
    return true;
  } catch {
    return false;
  }
